# Persistence of Intraplate Heat-Flow Anomalies and Tectonic Distance Analysis

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18892833.svg)](https://doi.org/10.5281/zenodo.18892833)

This repository contains the full reproducible analysis pipeline used in the study:

## Persistence of intraplate heat-flow anomalies far from tectonic boundaries

The project investigates the spatial relationship between terrestrial heat-flow measurements and tectonic structures, with a focus on intraplate anomalies that persist beyond classical tectonic controls.

All results reported in the manuscript are fully reproducible from this pipeline, and all figures correspond to versioned outputs included in this repository.

---

# Correspondence

**Cristiano Gaujac**
Independent Researcher
📧 [papers.gaujac@gmail.com](mailto:papers.gaujac@gmail.com)

---

# 1 Repository Structure

heatflow-intraplate-residuals/
│
├── pipeline/                         # Jupyter notebooks and analysis scripts
│
├── public_release/                  # Frozen reproducible outputs generated by the pipeline
│ │
│ ├── figures_main/                  # Main manuscript figures
│ ├── figures_supplementary_methods/ # Supplementary Methods figures
│ ├── figures_supplementary_results/ # Supplementary Results figures
│ │
│ ├── pipeline_kmeans/               # Final pipeline numerical outputs
│ ├── qa/                            # QA diagnostics and robustness tests
│ ├── flow/                          # QA workflow diagrams and pipeline logic
│ │
│ ├── env/                           # Computational environment audit
│ │ ├── env_audit.md
│ │ ├── pip_list.txt
│ │ ├── pip_freeze.txt
│ │ ├── path_resolution.txt
│ │ └── jupyter_version.txt
│ │
│ └── freeze/                        # Frozen pipeline reproducibility manifest
│
├── public_release_package/          # Curated export package (submission-ready figures)
│
├── data/                            # Input datasets (not redistributed when restricted)
│
├── environment.yml                  # Conda environment specification
├── CITATION.cff                     # Citation metadata
├── LICENSE                          # MIT license
├── .gitignore
└── README.md

---

# 2 Reproducibility

## 2.1 Create environment (recommended)

```bash
conda env create -f environment.yml
conda activate heatflow-intraplate-residuals
```

---

## 2.2 Run the pipeline

```bash
cd pipeline
jupyter lab
```

Open:

```
pipeline_heatflow_vs_tectonics.ipynb
```

---

## 2.3 Outputs

All figures, tables and reproducibility artifacts generated by the pipeline are written to:

```
public_release/
```

---

# 3 Frozen Results

The directory `public_release/` contains the canonical frozen outputs generated by the analysis pipeline.

These include:

• final figures used in the manuscript
• robustness diagnostics
• QA fragmentation tests
• pipeline reproducibility manifests
• computational environment audits

All frozen outputs can be cryptographically fingerprinted (SHA-256) to guarantee immutability and traceability.

---

# 4 Publication Figures

Publication figures are organized as:

```
public_release/
├── figures_main/
├── figures_supplementary_methods/
└── figures_supplementary_results/
```

These correspond exactly to the figures cited in the manuscript.

---

# 5 Reproducibility Records

The repository includes dedicated reproducibility artifacts:

```
public_release/env/
public_release/freeze/
```

`public_release/env/` contains the computational environment audit generated by the pipeline.

`public_release/freeze/` contains the frozen reproducibility manifest that records canonical dataset hashes and pipeline configuration.

These files allow reconstruction of the exact computational environment used in the analysis.

---

# 6 Data Sources

Main datasets used in this study:

• IHFC Global Heat Flow Database (2024 release)
• GEM Global Active Faults database
• PB2002 plate boundary model

All datasets are cited in the manuscript.

Raw data are not redistributed when licensing restrictions apply.

---

# 7 Reproducing Figures

Running the full pipeline reproduces:

• main manuscript figures
• distance–heat flow statistical analyses
• robustness tests and sensitivity analyses
• supplementary figures and diagnostics

Generated outputs are written to:

```
public_release/
```

---

# 8 Reproducibility and Traceability

This project follows strict reproducibility practices:

• deterministic analytical pipeline
• versioned figure outputs
• cryptographic fingerprints (SHA-256)
• environment audit
• frozen reproducibility manifest

These measures ensure that all reported results are fully traceable and reproducible.

---

# 9 Code Availability

The complete reproducible pipeline is available in this repository.

A frozen version corresponding to the manuscript is archived at:
https://doi.org/10.5281/zenodo.18892833

---

# 10 License and Data Use

This repository is provided for scientific reproducibility and peer verification.

The code is released under the **MIT License**.

Original datasets remain subject to their respective licenses and are cited in the manuscript. Redistribution of restricted datasets may not be permitted.